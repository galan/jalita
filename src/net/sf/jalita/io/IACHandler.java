 ...
 * <LI><A HREF="ftp://ds.internic.net/rfc/rfc1073.txt">1073 Telnet Window Size
 * Option</A>
 ...
 * <A HREF="ftp://ds.internic.net/rfc/rfc1416.txt">1416 Telnet Authentication
 * Option</A><BR>
 ...
 * NCSA telnet, and a server side implementation called key, a mud-like system
 * completely written in Java) I realized the problems we are facing regarding
...
class IACHandler {
	...
	/**
	 * Telnet readin buffer Here its implemented guys. Open your eyes upon this
	 * solution. The others take a one byte solution :)
	 */
	...
	public IACHandler(InputStream in, OutputStream out) throws IOException {
	...
	public void requestEchoModeInit() throws IOException {
		sendCommand(WONT, ECHO, true);
		sendCommand(DO, ECHO, true);
	}
	...
	public int getTerminalHeight() throws IOException {
		if(m_Height == 0) doCharacterModeInit();
		return m_Height;
	}
	
	public int getTerminalWidth() throws IOException{
		if(m_Width == 0) doCharacterModeInit();
		return m_Width;
	}
	 ...
	 * 
	 * @param buf
	 *            int [] that represents the first byte that followed the IAC
	 *            first.
	...
	/**
	 * Method that reads a NawsSubnegotiation that ends up with a IAC SE If the
	 * measurements are unbelieveable it switches to the defaults.
	 */
	 ...
	 * SE If no Terminal is valid, we switch to the dumb "none" terminal.
	 */
	private void handleTTYPE() throws IOException {
		...
		setNegotiatedTerminalType(tmpstr);
	}// handleTTYPE

	private void setNegotiatedTerminalType(String tmpstr) {
		termimalType = tmpstr;
	}
	...
	/*
	 * The characters following a "type" up to the next "type" or VALUE specify
	 * the variable name.
	 * 
	 * If a "type" is not followed by a VALUE (e.g., by another VAR, USERVAR, or
	 * IAC SE) then that variable is undefined.
	 */
	...
	/*
	 * The characters following a VALUE up to the next "type" specify the value
	 * of the variable. If a VALUE is immediately followed by a "type" or IAC,
	 * then the variable is defined, but has no value. If an IAC is contained
	 * between the IS and the IAC SE, it must be sent as IAC IAC.
	 */
	...
	/**
	 * Method that sends a TTYPE Subnegotiation Request. IAC SB TERMINAL-TYPE
	 * SEND
	 */
	...
	/**
	 * Method that sends a LINEMODE MODE Subnegotiation request. IAC LINEMODE
	 * MODE MASK SE
	 */
	...
	/**
	 * Method that sends a NEW-ENVIRON SEND subnegotiation request for default
	 * variables and user variables. IAC SB NEW-ENVIRON SEND VAR USERVAR IAC SE
	 */
	...
	/**
	 * Method that reads a subnegotiation String, one of those that end with a
	 * IAC SE combination. A maximum length is observed to prevent overflow.
	 * 
	...
	/**
	 * Method that mutates the wait status of an option in negotiation. We need
	 * the wait status to keep track of negotiation in process. So we cant miss
	 * if we started out or the other and so on.
	 * 
	 * @param WHAT
	...
	/**
	 * Method that sends the AreYouThere question of the telnet protocol
	 * specification
	 */
	 ...
	 *            int [] to write
	 */
	private void write(int[] sequence) throws IOException {
